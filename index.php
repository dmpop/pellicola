...
<body>
	 ...
	 */
		...
		if (isset($file)) {
			...
			if ($show_raw) {
				if (!empty($raw_file)) {
					echo "<h1>" . $file_path['filename'] . " <a href='download.php?file=" . $raw_file[0] . "'><img style='vertical-align: baseline;' alt='" . L::img_link_raw . "' title='" . L::img_link_raw . "' src='svg/raw.svg' height='22'/></a></h1>";
				} else {
			...
			if ($download) {
				echo '<div class="center"><a href="download.php?file=' . htmlentities($file) . '"><img style="max-width: 100%; border-radius: 7px;" src="' . htmlentities($tim) . '" alt="' . $file_path['filename'] . '" title="' . $file_path['filename'] . '"></a><div class="caption">' . $comment . ' ' . $description . '</div><div class="caption">' . $exif_info . '<a href="delete.php?file=' . $file . $raw . '"><img style="margin-left: 1em;" src="svg/bin.svg" alt="' . L::img_delete . '" title="' . L::img_delete . '" /></a></div>';
			} else {
